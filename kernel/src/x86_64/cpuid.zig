...
pub fn isRunningUnderQEMU() bool {
    // Method 1: Check hypervisor vendor ID (CPUID leaf 0x40000000)
    const hypervisor_info = cpuid(0x40000000, 0);

    // Check if hypervisor CPUID leaf is supported
    if (hypervisor_info.eax >= 0x40000000) {
        // Get hypervisor vendor string from leaves 0x40000000
        const vendor = cpuid(0x40000000, 0);

        // QEMU returns "TCGTCGTCGTCG" (12 bytes in EBX, ECX, EDX)
        // Check for TCG (Tiny Code Generator) signature
        if (vendor.ebx == 0x54474354 and // "TCGT"
            vendor.ecx == 0x54474354 and // "CGCT"
            vendor.edx == 0x47544347)
        { // "GTCG"
            return true;
        }

        // Also check for KVM signature "KVMKVMKVM\0\0\0"
        if (vendor.ebx == 0x4b4d564b and // "KVMK"
            vendor.ecx == 0x564b4d56 and // "VMKV"
            vendor.edx == 0x0000004d)
        { // "M\0\0\0"
            return true;
        }
    }

    // Method 2: Check for QEMU-specific CPU model strings
    // CPUID leaf 0x80000002-0x80000004 contains the processor brand string
    const leaf_0x80000000 = cpuid(0x80000000, 0);
    if (leaf_0x80000000.eax >= 0x80000004) {
        // Get processor brand string (48 bytes across 3 leaves)
        const brand1 = cpuid(0x80000002, 0);
        const brand2 = cpuid(0x80000003, 0);

        // Check for "QEMU" in the brand string
        // "QEMU" = 0x554D4551
        if (brand1.eax == 0x554D4551 or // "QEMU"
            brand1.ebx == 0x554D4551 or
            brand1.ecx == 0x554D4551 or
            brand1.edx == 0x554D4551 or
            brand2.eax == 0x554D4551 or
            brand2.ebx == 0x554D4551 or
            brand2.ecx == 0x554D4551 or
            brand2.edx == 0x554D4551)
        {
            return true;
        }
    }

    // Method 3: Check for missing/unusual CPU features that indicate virtualization
    // QEMU often has inconsistent feature flags
    const features = cpuid(1, 0);
    const has_hypervisor_bit = (features.ecx & (1 << 31)) != 0; // Hypervisor present bit

    return has_hypervisor_bit;
}
