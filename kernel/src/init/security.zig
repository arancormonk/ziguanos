...
pub fn initFull() !void {
    ...
    io_security.initializeFull() catch |err| {
        serial.println("[KERNEL] WARNING: Full I/O security init failed: {s}", .{@errorName(err)});
        serial.flush();
    ...
    io_security.testHardwareIOPB() catch |err| {
        serial.println("[KERNEL] WARNING: Hardware IOPB test failed: {s}", .{@errorName(err)});
        serial.flush();
    };

    // Initialize full interrupt security now that PMM is available
    interrupt_security.fullInit() catch |err| {
        serial.println("[KERNEL] WARNING: Full interrupt security init failed: {s}", .{@errorName(err)});
        serial.flush();
    ...
    stack_security.initializeAdvancedFeatures() catch |err| {
        serial.println("[KERNEL] ERROR: Advanced stack protection init failed: {s}", .{@errorName(err)});
        serial.flush();
        return err;
    };
    ...
    stack_security.setupStackGuardPages() catch |err| {
        serial.println("[KERNEL] ERROR: Stack guard pages setup failed: {s}", .{@errorName(err)});
        serial.flush();
        return err;
    };
}
